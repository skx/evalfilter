...
func (e *Eval) Prepare() error {
	...
	//
	// Attempt to optimize the code, running
	// multiple passes until no changes are
	// made.
	//
	// We do this so that each optimizer run only
	// has to try one thing at a time.
	//
	for e.Optimize() {
		// rewrite
	}
...
// Optimize the generated byecode, if we can.
//
...
//  000006 OpMul
//
// That can be replaced by "OpPush 6", "NOP", NOP, NOP
//
// TODO: Drop the Nop.
//
func (e *Eval) Optimize() bool {
	...
	type Constants struct {
		// offset is where we found this constant
		// instruction.
		offset int

		// value is the (integer) constant value.
		value int
	}
	...
	for ip < ln {
		...
		case code.OpEqual:
			...
			// we can collapse the equality test into
		...
		default:
			...
			// and instruction that wasn't a constant
			// load, and wasn't something we can fold.
			//
